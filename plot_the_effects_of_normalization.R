plot_the_effects_of_normalization <- function(DGEList = obj, specific.genes = c("sel", "aurA", "roX1"), random.genes = NA){
  ...
  data <- obj$cpm_trimmed_normalized
  data  <- apply(data,2,as.numeric)
 
  ##create a time point vector that fits the main data
  t <- sapply(strsplit(colnames(log2data),"t"), function(x){x[!x ==""]})
  t <- as.numeric(t)
  ...
  for(i in 1:length(genes)){
    ...
    plot(realt,data[genes[i],]*obj$samples$norm.factors, type= "b", main = paste("normalized:",obj$genes$symbol[genes[i]]), log = "y", xlab = "time")
